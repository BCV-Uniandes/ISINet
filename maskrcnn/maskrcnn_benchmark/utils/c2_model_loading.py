...
def _rename_basic_resnet_weights(layer_keys):
    ...
    layer_keys = [
        k.replace("downsample.0.gn.s", "downsample.1.weight")
        for k in layer_keys
    ]
    layer_keys = [
        k.replace("downsample.0.gn.bias", "downsample.1.bias")
        for k in layer_keys
    ]
...
def _rename_fpn_weights(layer_keys, stage_names):
    for mapped_idx, stage_name in enumerate(stage_names, 1):
        ...
        layer_keys = [
            k.replace(
                "fpn.layer{}.sum".format(stage_name),
                "fpn_layer{}".format(mapped_idx),
            )
            for k in layer_keys
        ]
