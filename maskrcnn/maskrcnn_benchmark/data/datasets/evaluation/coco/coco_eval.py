...
def prepare_for_coco_segmentation(predictions, dataset):
    ...
    for image_id, prediction in tqdm(enumerate(predictions)):
        ...
        scores = prediction.get_field("scores").tolist()
        labels = prediction.get_field("labels").tolist()
        topk_scores = np.asarray(prediction.get_field("scores_topk")).tolist()
        topk_indices = np.asarray(
            prediction.get_field("indices_topk")
        ).tolist()
...
def evaluate_predictions_on_coco(
    ...
    coco_dt = (
        coco_gt.loadRes(str(json_result_file)) if coco_results else COCO()
    )
...
class COCOResults(object):
    METRICS = {
        "bbox": ["AP", "AP50", "AP75", "APs", "APm", "APl", "AR@10", "AR@1"],
        "segm": ["AP", "AP50", "AP75", "APs", "APm", "APl", "AR@10", "AR@1"],
        "box_proposal": [
            "AR@100",
            "ARs@100",
            "ARm@100",
